...
public:
	...
	///        If the result is not available yet, the method can react by returning an incomplete result (pending status).
	...
	///
	/// The default implementation just returns the data object itself as the evaluation result.
	virtual Future<PipelineFlowState> evaluateAsync(TimePoint time) {
		return Future<PipelineFlowState>::createImmediate(PipelineFlowState(this, objectValidity(time)));
	}
