...
template<>
inline QDataStream& operator<<(QDataStream& stream, const Point_2<double>& v) {
...
// performs no automatic precision conversion of floating point numbers.
template<>
inline QDataStream& operator>>(QDataStream& stream, Point_2<double>& v) {
	Point_2<float> floatValue;
	stream >> floatValue;
